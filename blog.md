...
docker run -d --name gemfire-locator \
-p 10334:10334 \
--network host \
gemfire/gemfire-all:10.1-jdk21 \
gfsh start locator --name=locator --hostname-for-clients=localhost
--name gemfire-locator: Container name.
-p 10334:10334: Maps the locator's discovery port.
--network host: Allows your Spring Boot app on the host to connect easily.
--hostname-for-clients=localhost: Tells clients (like our Spring Boot app) to connect via localhost.
b. Start a GemFire Server:
The server is where your data will actually be stored in memory.

Bash

docker run -d --name gemfire-server \
-p 40404:40404 \
--network host \
gemfire/gemfire-all:10.1-jdk21 \
gfsh start server --name=server --locators=localhost[10334]
-p 40404:40404: Maps the server's client port.
--locators=localhost[10334]: Tells the server to connect to the locator we just started.
c. Connect to gfsh and Create the Region:
Now, connect to the gfsh (GemFire Shell) and define the region where DataItem objects will be cached.
